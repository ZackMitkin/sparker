...
class Blocking(object):
    @staticmethod
    def create_blocks_clusters(profiles: RDD, clusters, separator_ids=None, keys_to_exclude=None,
        ...
        entropies = dict(map(lambda cluster: (
            cluster.cluster_id, cluster.entropy), clusters))
        cluster_map = dict([(k, c.cluster_id)
                           for c in clusters for k in c.keys])

        def create_keys(profile):
            try:
                dataset = str(profile.source_id) + str(cluster_name_separator)
                non_empty_attributes = list(
                    filter(lambda a: not a.is_empty(), profile.attributes))
                valid_keys = list(filter(lambda x: len(x[1]) > 0,
                                         map(lambda x: ("", "") if x.key in attributes_to_exclude else (x.key, x.value),
                                             non_empty_attributes)))
                split = list(
                    map(lambda x: (x[0], re.split('\W+', x[1].lower())), valid_keys))
        ...
        print("COUNT", profiles.count())
        tokens_per_profile = profiles.map(create_keys)

        token_profile = tokens_per_profile.flatMap(
            lambda x: [(y, x[0]) for y in x[1]])

        profile_per_key = token_profile.groupByKey().filter(
            lambda x: len(x[1]) > 1)
